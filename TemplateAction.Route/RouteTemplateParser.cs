...
namespace TemplateAction.Route
{
    public class RouteTemplateParser
    {
        ...
        public static RouteTemplate Parse(string routeTemplate)
        {
            ...
            {
                throw new ArgumentException(routeTemplate);
            }

            TemplateParserContext context = new TemplateParserContext(routeTemplate);
            List<RouteTemplateSegment> segments = new List<RouteTemplateSegment>();
        ...
        private static bool ParseSegment(TemplateParserContext context, List<RouteTemplateSegment> segments)
        {
            RouteTemplateSegment segment = new RouteTemplateSegment();
            ...
            while (true)
            {
                if (context.Current == OpenBrace)
                {
                    if (!context.Next())
                    {
                        context.Error = "TemplateRoute_MismatchedParameter";
                        return false;
                    }
        ...
        private static bool ParseParameter(TemplateParserContext context, RouteTemplateSegment segment)
        {
            context.Mark();
                ...
                if (context.Current == OpenBrace)
                {
                    ...
                    if (context.Next())
                    {
                        if (context.Current != OpenBrace)
                        {
                            ...
                            context.Error = "TemplateRoute_UnescapedBrace";
                            return false;
                        }
                    }
                    else
            ...
            var templatePart = RouteInlineParameterParser.ParseRouteParameter(decoded);

            if (templatePart.IsCatchAll && templatePart.IsOptional)
            {
                context.Error = "TemplateRoute_CatchAllCannotBeOptional";
                return false;
            }
            ...
            if (templatePart.IsOptional && templatePart.DefaultValue != null)
            {
                ...
                context.Error = "TemplateRoute_OptionalCannotHaveDefaultValue";
                return false;
            }
        ...
        private static bool ParseLiteral(TemplateParserContext context, RouteTemplateSegment segment)
        {
            context.Mark();
            ...
            {
                segment.Parts.Add(RouteTemplatePart.CreateLiteral(decoded));
                return true;
            }
            else
            {
        ...
        private static bool IsAllValid(TemplateParserContext context, List<RouteTemplateSegment> segments)
        {
            ...
            for (var i = 0; i < segments.Count; i++)
            {
                ...
                for (var j = 0; j < segment.Parts.Count; j++)
                {
                    ...
                    {
                        context.Error = "TemplateRoute_CatchAllMustBeLast";
                        return false;
                    }
                }
            }
        ...
        private static bool IsSegmentValid(TemplateParserContext context, RouteTemplateSegment segment)
        {
            ...
            for (var i = 0; i < segment.Parts.Count; i++)
            {
                ...
                {
                    context.Error = "TemplateRoute_CannotHaveCatchAllInMultiSegment";
                    return false;
                }
            }
                ...
                if (part.IsParameter && part.IsOptional && segment.Parts.Count > 1)
                {
                    ...
                    if (i == segment.Parts.Count - 1)
                    {
                        ...
                        else
                        {
                            ...
                            // period (.).
                            context.Error = string.Format("{0}{1}",
                                part.Name,
            ...
            for (var i = 0; i < segment.Parts.Count; i++)
            {
                ...
                {
                    context.Error = "TemplateRoute_CannotHaveConsecutiveParameters";
                    return false;
                }
