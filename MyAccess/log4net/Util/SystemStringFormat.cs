...
namespace MyAccess.log4net.Util
{
	...
	public sealed class SystemStringFormat
	{
		...
		private static string StringFormat(IFormatProvider provider, string format, params object[] args)
		{
			...
			{
				MyAccess.log4net.Util.LogLog.Warn(declaringType, "Exception while rendering format ["+format+"]", ex);
				return StringFormatError(ex, format, args);
			}
#if !NET_2_0 && !MONO_2_0 && !MONO_3_5 && !MONO_4_0
			catch
			{
				MyAccess.log4net.Util.LogLog.Warn(declaringType, "Exception while rendering format ["+format+"]");
		...
		private static string StringFormatError(Exception formatException, string format, object[] args)
		{
			...
			{
				MyAccess.log4net.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling", ex);
				return "<log4net.Error>Exception during StringFormat. See Internal Log.</log4net.Error>";
			}
#if !NET_2_0 && !MONO_2_0 && !MONO_3_5 && !MONO_4_0
			catch
			{
				MyAccess.log4net.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling");
				return "<log4net.Error>Exception during StringFormat. See Internal Log.</log4net.Error>";
			}
#endif
		}
