...
using MyAccess.log4net.Config;
using MyAccess.log4net.Util;
using MyAccess.log4net.Repository;

namespace MyAccess.log4net.Core
{
	/// <summary>
	...
	public class DefaultRepositorySelector : IRepositorySelector
	{
		...
		/// an appropriate value would be <see cref="MyAccesslog4net.Repository.Hierarchy.Hierarchy"/>.
		/// </para>
		/// </remarks>
		...
		public DefaultRepositorySelector(Type defaultRepositoryType)
		{
			...
			{
				throw MyAccess.log4net.Util.SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", defaultRepositoryType, "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType + "] out of range. Argument must implement the ILoggerRepository interface");
			}
		...
		/// to create can be overridden by specifying the <see cref="MyAccesslog4net.Config.RepositoryAttribute"/> 
		/// attribute on the <paramref name="repositoryAssembly"/>.
		/// </para>
		/// <para>
		/// The default values are to use the <see cref="MyAccesslog4net.Repository.Hierarchy.Hierarchy"/> 
		...
		/// any <see cref="MyAccesslog4net.Config.ConfiguratorAttribute"/> attributes defined on
		...
		/// <see cref="MyAccesslog4net.Config.RepositoryAttribute"/> attribute on the 
		...
		/// configured using any <see cref="MyAccesslog4net.Config.ConfiguratorAttribute"/> 
		...
		/// Also the <see cref="MyAccesslog4net.Config.RepositoryAttribute"/> attribute on the
		...
		/// </summary>
		/// <param name="assembly">The assembly that has a <see cref="MyAccesslog4net.Config.RepositoryAttribute"/>.</param>
		...
		private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType)
		{
			...
			try
			{
				...
				object[] repositoryAttributes = Attribute.GetCustomAttributes(assembly, typeof(MyAccess.log4net.Config.RepositoryAttribute), false);
				...
				else
				{
					...
					MyAccess.log4net.Config.RepositoryAttribute domAttr = repositoryAttributes[0] as MyAccess.log4net.Config.RepositoryAttribute;
		...
		/// </summary>
		/// <param name="assembly">The assembly containing <see cref="MyAccesslog4net.Config.ConfiguratorAttribute"/>
		...
		private void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(MyAccess.log4net.Config.ConfiguratorAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				...
				foreach(MyAccess.log4net.Config.ConfiguratorAttribute configAttr in configAttributes)
				{
					if (configAttr != null)
					{
						try
		...
		private void LoadPlugins(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(MyAccess.log4net.Config.PluginAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				foreach(MyAccess.log4net.Plugin.IPluginFactory configAttr in configAttributes)
				{
					try
					{
		...
		private void LoadAliases(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(MyAccess.log4net.Config.AliasRepositoryAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				foreach(MyAccess.log4net.Config.AliasRepositoryAttribute configAttr in configAttributes)
				{
					try
					{
