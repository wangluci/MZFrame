...
namespace MyAccess.WordSegment.Match
{
    public class ChsFullTextMatch: IChsFullTextMatch
    {
        ...
        Framework.AppendList<Node> _LeafNodeList = new WordSegment.Framework.AppendList<Node>();
        Dict.PositionLength[] _PositionLengthArr;
        int _InputStringLength;
        int _PositionLengthArrCount;

        List<Dict.PositionLength[]> _AllCombinations = new List<WordSegment.Dict.PositionLength[]>();
        ...
        {
            LinkedList<Dict.PositionLength> result = new LinkedList<WordSegment.Dict.PositionLength>();
        ...
        /// <returns></returns>
        private Node[] GetLeafNodeArray(WordSegment.Dict.PositionLength[] positionLenArr, int orginalTextLength, int count)
        {
            ...
            for (int i = 1; i < count; i++)
            {
                if (positionLenArr[i].Position >= lastRightBoundary)
                {
                    ...
                    int c = i - lastIndex;
                    WordSegment.Dict.PositionLength[] arr = new WordSegment.Dict.PositionLength[c];
        ...
        /// <returns></returns>
        private Node[] GetLeafNodeArrayCore(WordSegment.Dict.PositionLength[] positionLenArr, int orginalTextLength, int count)
        {
            _LeafNodeList.Clear();
        ...
        public SuperLinkedList<WordInfo> Match(WordSegment.Dict.PositionLength[] positionLenArr, string orginalText, int count)
        {
            if (_Options == null)
            {
            ...
            foreach (Node node in leafNodeArray)
            {
                ...
                Dict.PositionLength[] comb = new WordSegment.Dict.PositionLength[node.AboveCount];
            ...
            {
                Dict.PositionLength[] comb = new WordSegment.Dict.PositionLength[orginalText.Length];

                for (int i = 0; i < comb.Length; i++)
                {
                    WordSegment.Dict.PositionLength pl = new WordSegment.Dict.PositionLength(i, 1, new WordAttribute(orginalText[i].ToString(), POS.POS_UNK, 0));
                    pl.Level = 3;
                    comb[i] = pl;
                }
