...
using MyAccess.WordSegment.Framework;
using MyAccess.WordSegment.Dict;
...
namespace MyAccess.WordSegment
{
    ...
    //@"[０-９\d]+|[^ａ-ｚＡ-Ｚa-zA-Z0-9０-９\u4e00-\u9fa5]|[ａ-ｚＡ-Ｚa-zA-Z]+|[\u4e00-\u9fa5]+";
        ...
        internal static ICustomSource _source = null;
        ...
        /// <returns></returns>
        public static List<SearchWordResult> GetLikeKey(string keyword)
        {
            Init();
            return _WordDictionary.Search(keyword.Trim());
        }
        /// <summary>
        /// 合并英文专用词。
        ...
        private bool MergeEnglishSpecialWord(string orginalText, SuperLinkedList<WordInfo> wordInfoList, ref SuperLinkedListNode<WordInfo> current)
        {
            ...
            if (last >= 0)
            {
                ...
                WordInfo newWordInfo = new WordInfo(new WordSegment.Dict.PositionLength(first, last - first, 
        ...
        private SuperLinkedList<WordInfo> GetInitSegment(string text)
        {
            ...
            Framework.Lexical lexical = new WordSegment.Framework.Lexical(text);
            ...
            while (cur != null)
            {
                ...
                switch (cur.Value.WordType)
                {
                    case WordType.SimplifiedChinese:
                        ...
                        WordSegment.Framework.AppendList<Dict.PositionLength> pls = _WordDictionary.GetAllMatchs(inputText, _Options.ChineseNameIdentify);
                        WordSegment.Match.ChsFullTextMatch chsMatch = new WordSegment.Match.ChsFullTextMatch(_WordDictionary);
        ...
        public ICollection<WordInfo> DoSegment(string text, Match.MatchOptions options, Match.MatchParameter parameters)
        {
            ...
            try
            {
                Dict.DictionaryLoader.Lock.Enter(WordSegment.Framework.Lock.Mode.Share);
                _Options = options;
        ...
        {
            _WordDictionary = new WordSegment.Dict.WordDictionary();
            string dir = Setting.PanGuSettings.Config.GetDictionaryPath();
            if (_source != null)
            {
                _WordDictionary.Load(_source.DictFrom());
            }
            else
            {
            ...
            if (_source != null)
            {
                _DictLoader = new WordSegment.Dict.CustomDictionaryLoader(_source, Setting.PanGuSettings.Config.GetDictionaryPath());
            }
            else
            {
                _DictLoader = new WordSegment.Dict.FileDictionaryLoader(Setting.PanGuSettings.Config.GetDictionaryPath());
            }
        ...
        private static void InitInfinitiveVerbTable()
        {
            ...
            using (System.IO.StringReader sr = new System.IO.StringReader(MyAccess.WordSegment.Dict.VerbTable.InfinitiveVerbTable))
            {
        ...
        public static void InitWith(ICustomSource source)
        {
            if (_source == null)
            {
                _source = source;
        ...
        public static void Init(string fileName)
        {
            lock (_LockObj)
            {
                ...
                if (fileName == null)
                {
                    Setting.SettingLoader loader = new WordSegment.Setting.SettingLoader();
                }
                else
                {
                    Setting.SettingLoader loader = new WordSegment.Setting.SettingLoader(fileName);
                }
                ...
                _Wildcard = new WordSegment.Dict.Wildcard(Setting.PanGuSettings.Config.MatchOptions,
